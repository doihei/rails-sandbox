...
export default class extends Controller {
  static targets = ["list", "loading", "error", "pagination"];
  ...
  async loadMore() {
    ...
    await this.fetchArticles(this.#endCursor);
  }

  async fetchArticles(after = null) {
    ...
    try {
      ...
      this.paginationTarget.hidden = !this.#hasNextPage;
    } catch (err) {
  ...
  #append(articles) {
    if (!articles.length && !this.listTarget.children.length) {
    ...
    this.listTarget.insertAdjacentHTML(
      "beforeend",
      articles.map((a) => this.#articleHTML(a)).join(""),
    );
  }
